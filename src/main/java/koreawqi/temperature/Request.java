    ...
    public String getTemperatureJSONString() throws IOException {
        ...
        while (timetableString.contains("<") | timetableString.contains(">")) {
            timetableString = timetableString
                    .replace(timetableString.substring(timetableString.indexOf("<"), timetableString.indexOf(">")+1), "");
        }
        timetableString = timetableString.substring(timetableString.indexOf("="), timetableString.indexOf(";"));

        while(timetableString.contains(" ")||timetableString.contains("=")||timetableString.contains("\"")) timetableString = timetableString.replace("=", "").replace("\"", "").replace(" ", "");

        String jsonDataString = "";
        jsonDataString += "{";
        jsonDataString += "\"" + "date" + "\"" + ":" + "\"" + timetableString.substring(0,timetableString.indexOf("+")) + "\",";
        jsonDataString += "\"" + "time" + "\"" + ":" + "\"" + timetableString.substring(timetableString.indexOf("+")+1,timetableString.length()) + "\",";
                ...
                for (int k = 0; k < table_01Elements.get(i).childNode(j).childNodeSize(); k++) {
                    ...
                    try {
                        jsonDataString += "{" + "\"name\":" + "\""
                                ...
                                + "\"," + "" + "\"temp\":"
                                + (table_01Elements.get(i).childNode(j).childNode(k).childNode(4).toString().contains(
                                        ".") ? table_01Elements.get(i).childNode(j).childNode(k).childNode(4).toString()
                                                : "\"" + table_01Elements.get(i).childNode(j).childNode(k).childNode(3)
